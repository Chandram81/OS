...
function __Normalize-And-Export {
  ...
  $Unified | ConvertTo-Json -Depth 6 | Out-File -Encoding UTF8 $JsonPath
...
try {
  ...
  Add-Result -Id 'HP-14b' -Priority 'High' -Area 'BitLocker' -Check 'TPM+PIN protector' -Expected 'Present' -Actual 'Query failed' -Compliant $false
  Add-Why 'Mitigate stolen device + boot bypass' 'Bind decryption to something you have (TPM) + know (PIN)'
}
...
try {
  ...
  Add-Result -Id 'HP-06c' -Priority 'High' -Area 'VBS' -Check 'VBS runtime status' -Expected 'On (CG/HVCI running)' -Actual 'Query failed' -Compliant $false; Add-Why 'Validate not just configured but running' 'Close the “configured but not active” gap'
}
...
foreach ($r in $Results | Where-Object { $_.Id -eq 'LP-05' }) {
  $r.Why='Reduce outbound PII leakage'; $r.Purpose='Stop crash data leaving endpoints'
}

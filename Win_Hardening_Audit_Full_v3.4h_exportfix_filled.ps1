...
# ===== v3.4c update (generated 2025-11-11) =====
# - Normalized Check text (removed trailing periods; preserved casing)
# - De-duplicated Add-Result IDs (appended -R<n> suffix for collisions)
# - Inserted missing high-impact checks early in the audit flow
# - Introduced Results Policy: ID allowlist -> DYN-*, Prereq column, mandatory NEW-* set asserted
# ===========================================
...
# ===== Normalize-Results (fills Why, Purpose, Prereq, CompliantBool) =====
function Normalize-Results {
  param([Parameter(Mandatory=$true)][ref]$Results)

  $PrereqById = @{
    'NEW-TPM-01'='TPM present & ready'
    'NEW-TPM-02'='UEFI + Secure Boot capability'
    'NEW-WRM-01'='WinRM installed'
    'NEW-AL-01' ='AppLocker available (Ent/Edu)'
    'NEW-WDAC-01'='WDAC feature available'
    'NEW-DEF-01'='Windows Defender present'
    'NEW-TLS-01'='Schannel Server key present'
    'NEW-FW-01' ='Windows Firewall service present'
  }

  $WhyPurpose = @{
    'PowerShell'         = @('Trace admin/script activity','Visibility into script abuse')
    'AppLocker'          = @('Constrain code execution','Reduce malware/LOLBIN risk')
    'Firewall'           = @('Default-deny inbound; control egress','Reduce attack surface/exfiltration')
    'TLS'                = @('Remove legacy protocol/cipher risk','Prevent downgrade/weak crypto')
    'Account Policy'     = @('Resist brute force & spraying','Halt credential stuffing success')
    'WinRM'              = @('Harden remote mgmt channel','Eliminate HTTP/Basic listeners')
    'TPM/SecureBoot'     = @('Hardware root of trust','Block bootkits/credential theft')
    'Exploit Protection' = @('Enable memory mitigations','Mitigate ROP/memory corruption')
    'Defender'           = @('Maintain endpoint protection','Keep sensor integrity & coverage')
    'WDAC'               = @('Whitelist-only code integrity','Block untrusted binaries/drivers')
    'Accounts'           = @('Enforce least privilege','Limit lateral movement blast radius')
    'Services'           = @('Disable risky legacy services','Reduce remote tampering surface')
    'Browser'            = @('Block phishing/drive-by','Reduce user-originated compromise')
    'Telemetry'          = @('Limit diagnostic data','Reduce PII exposure')
    'RDP'                = @('Harden remote desktop','Prevent weak auth/session abuse')
    'LSASS'              = @('Protect credentials at rest','Prevent LSASS credential theft')
    'BitLocker'          = @('Encrypt data at rest','Reduce exfil risk on loss')
    'Auditing'           = @('Capture security events','Enable forensics & detection')
    'Logging'            = @('Retain logs with signal','Support IR and compliance')
    'Name Resolution'    = @('Disable spoofable protocols','Reduce LLMNR/NetBIOS MITM')
    'SMB'                = @('Require signing/modern dialects','Prevent relay/legacy exposure')
    'Office'             = @('Restrict macro abuse','Reduce phishing payload risk')
    'Privacy'            = @('Reduce unnecessary data sharing','Meet regulatory expectations')
    'Attachment'         = @('Enforce safe attachment handling','Reduce malware delivery risk')
    'AutoPlay'           = @('Disable auto-execution','Prevent USB-borne malware')
    'Bloat'              = @('Remove unnecessary software','Reduce attack surface')
    'OS'                 = @('Baseline OS hardening','Close common misconfig gaps')
    'Boot Security'      = @('Secure the boot chain','Prevent tampered startup')
    'Application Control'= @('Constrain what can run','Block unapproved code paths')
  }

  for ($i=0; $i -lt $Results.Value.Count; $i++) {
    $r = $Results.Value[$i]
    if (-not ($r.PSObject.Properties.Name -contains 'Prereq'))        { Add-Member -InputObject $r -NotePropertyName Prereq        -NotePropertyValue $null }
    if (-not ($r.PSObject.Properties.Name -contains 'CompliantBool')) { Add-Member -InputObject $r -NotePropertyName CompliantBool -NotePropertyValue $null }
    if (-not ($r.PSObject.Properties.Name -contains 'Why'))           { Add-Member -InputObject $r -NotePropertyName Why           -NotePropertyValue $null }
    if (-not ($r.PSObject.Properties.Name -contains 'Purpose'))       { Add-Member -InputObject $r -NotePropertyName Purpose       -NotePropertyValue $null }

    $v = ($r.Compliant | ForEach-Object { $_.ToString().Trim().ToUpper() })
    if     ($v -eq 'PASS' -or $v -eq 'TRUE')  { $r.CompliantBool = $true }
    elseif ($v -eq 'FAIL' -or $v -eq 'FALSE') { $r.CompliantBool = $false }
    elseif ($null -eq $r.CompliantBool)       { $r.CompliantBool = $false }

    if (-not $r.Prereq -or ($r.Prereq -is [string] -and $r.Prereq.Trim() -eq '')) {
      if ($PrereqById.ContainsKey($r.Id)) { $r.Prereq = $PrereqById[$r.Id] } else { $r.Prereq = 'None' }
    }

    if (-not $r.Why     -and $WhyPurpose.ContainsKey($r.Area)) { $r.Why     = $WhyPurpose[$r.Area][0] }
    if (-not $r.Purpose -and $WhyPurpose.ContainsKey($r.Area)) { $r.Purpose = $WhyPurpose[$r.Area][1] }

    $Results.Value[$i] = $r
  }
}
# ===== End Normalize-Results =====
...
# ===== Results Policy (v3.4b, 2025-11-11) =====
function Invoke-ResultsPolicy {
  param(
    [Parameter(Mandatory=$true)]
    [ref]$Results
  )
  $ids = [System.Collections.Generic.HashSet[string]]::new()
  foreach ($r in $Results.Value) { [void]$ids.Add($r.Id) }

  $AllowedPrefixes = @('^MP-','^NEW-','^A\d+','^RDP-','^SMB','^GPO-','^WDAC','^TLS','^AC','^LDAP','^DEF','^FW','^EP','^TPM','^WRM','^AL-','^PS-','^BL-')
  function Test-AllowedId([string]$Id) { foreach ($p in $AllowedPrefixes) { if ($Id -match $p) { return $true } } return $false }

  $MandatoryIds = @('NEW-PS-01','NEW-PS-02','NEW-AL-01','NEW-FW-01','NEW-TLS-01','NEW-AC-01','NEW-WRM-01','NEW-TPM-01','NEW-TPM-02','NEW-EP-01','NEW-DEF-01','NEW-WDAC-01','NEW-LA-01','NEW-SVC-01','NEW-BRW-01','NEW-TEL-01')

  $PrereqMap = @{
    'NEW-TPM-01' = 'TPM present & ready';
    'NEW-TPM-02' = 'UEFI + SecureBoot capability';
    'NEW-WRM-01' = 'WinRM installed';
    'NEW-AL-01'  = 'AppLocker feature available (Enterprise/Education)';
    'NEW-WDAC-01'= 'WDAC feature available (Enterprise/Server)';
    'NEW-DEF-01' = 'Windows Defender present';
    'NEW-TLS-01' = 'Server Schannel registry path present';
    'NEW-FW-01'  = 'Windows Firewall service present';
  }

  foreach ($r in $Results.Value) {
    if (-not (Test-AllowedId $r.Id)) { $r.Id = 'DYN-' + $r.Id }
    if (-not ($r.PSObject.Properties.Name -contains 'Prereq')) {
      $pr = $null
      if ($PrereqMap.ContainsKey($r.Id)) { $pr = $PrereqMap[$r.Id] }
      if (-not $pr) { $pr = 'None' }
      Add-Member -InputObject $r -NotePropertyName 'Prereq' -NotePropertyValue $pr -Force
    }
  }

  $missing = @(); foreach ($m in $MandatoryIds) { if (-not ($ids.Contains($m))) { $missing += $m } }
  if ($missing.Count -gt 0) { Write-Error ('Mandatory checks missing: ' + ($missing -join ', ')); throw 'Blocking export due to missing mandatory checks' }
}
# ===== End Results Policy =====
...
# ===== v3.3 bootstrap: unified metadata & exit export hook (inserted after param) =====
function New-Result {
  param(
    [string]$Id,[string]$Title,[string]$Severity,[bool]$Passed,[string]$Evidence,
    [string]$Category = "General",[string]$Remediation = ""
  )
  switch -Regex ($Category) {
    "CredProt|LSASS" { $why="Prevents credential theft from memory."; $purpose="Protect LSASS and credential stores."; $impact="Credential dumping enables lateral movement."; }
    "Firewall"       { $why="Blocks unauthorized network access."; $purpose="Enforce firewall across profiles."; $impact="Open access expands breach blast radius."; }
    "Network"        { $why="Removes legacy/insecure protocols."; $purpose="Harden communications and name resolution."; $impact="Weak protocols allow interception/relay."; }
    "Identity"       { $why="Protects authentication integrity."; $purpose="Harden NTLM/UAC/SSO posture."; $impact="Weak identity controls enable impersonation."; }
    "Defender"       { $why="Stops malware and suspicious activity."; $purpose="Maintain Defender AV/Cloud/NRT."; $impact="Disabled AV permits undetected execution."; }
    "Remote Access"  { $why="Limits remote exploitation paths."; $purpose="Secure/disable RDP/RA."; $impact="Exposed RDP increases compromise risk."; }
    "Logging"        { $why="Enables detection and forensics."; $purpose="Capture event and PowerShell logs."; $impact="Missing logs hinder investigation."; }
    "Disk"           { $why="Protects data at rest."; $purpose="Ensure BitLocker encryption."; $impact="Lost devices may leak sensitive data."; }
    "OS"             { $why="Reduces local attack surface."; $purpose="Apply secure baseline settings."; $impact="Unhardened OS eases privilege abuse."; }
    default          { $why="Maintains baseline compliance."; $purpose="Align with org security standards."; $impact="Deviations weaken defense-in-depth."; }
  }
  [PSCustomObject]@{
    Id=$Id; Title=$Title; Category=$Category; Severity=$Severity; Passed=$Passed; Evidence=$Evidence;
    Why=$why; Purpose=$purpose; BusinessImpact=$impact; Remediation=$Remediation
  }
}

function Convert-LegacyResult {
  param([psobject]$r)
  $id = $r.Id
  $title = if($r.PSObject.Properties.Match('Title')){ $r.Title } elseif($r.PSObject.Properties.Match('Check')){ $r.Check } else { "" }
  $category = if($r.PSObject.Properties.Match('Category')){ $r.Category } elseif($r.PSObject.Properties.Match('Area')){ $r.Area } else { "General" }
  $severity = if($r.PSObject.Properties.Match('Severity')){ $r.Severity } elseif($r.PSObject.Properties.Match('Priority')){ $r.Priority } else { "Medium" }
  $passed = if($r.PSObject.Properties.Match('Passed')){ [bool]$r.Passed } elseif($r.PSObject.Properties.Match('Compliant')){ ($r.Compliant -eq 'PASS') } else { $false }
  $evidence = if($r.PSObject.Properties.Match('Evidence')){ $r.Evidence } else { ("Actual: {0} | Expected: {1}" -f ($r.Actual), ($r.Expected)) }
  $remed = if($r.PSObject.Properties.Match('Remediation')){ $r.Remediation } else { "" }
  return New-Result -Id $id -Title $title -Category $category -Severity $severity -Passed:$passed -Evidence $evidence -Remediation $remed
}

function __Normalize-And-Export {
  if (-not $Results) { return }
  $Unified = @()
  foreach ($r in $Results) {
    if ($r.PSObject.Properties.Match('Why') -and $r.PSObject.Properties.Match('Purpose') -and $r.PSObject.Properties.Match('BusinessImpact')) {
      if (-not $r.Why) { $r.Why = "Maintains baseline compliance." }
      if (-not $r.Purpose) { $r.Purpose = "Ensure control aligns with baseline." }
      if (-not $r.BusinessImpact) { $r.BusinessImpact = "Missing control increases attack exposure." }
      $Unified += $r
    } else {
      $Unified += (Convert-LegacyResult -r $r)
    }
  }
  if (-not $CsvPath) {
    $CsvPath = Join-Path -Path (Split-Path -Parent $MyInvocation.MyCommand.Path) -ChildPath "Win_Hardening_Audit_v3.csv"
  }
  if (-not $JsonPath) {
    $JsonPath = Join-Path -Path (Split-Path -Parent $MyInvocation.MyCommand.Path) -ChildPath "Win_Hardening_Audit_v3.json"
  }
  $Unified | 
# Enforce results policy before export
Invoke-ResultsPolicy -Results ([ref]$Results)


# ======= v3.4f export normalization (2025-11-11) =======
# Add CompliantBool, ensure Prereq is present, and backfill Why/Purpose prior to export
# Area-based rationale map (short, leadership-friendly)
$WhyPurposeByArea = @{
  'PowerShell'        = @('Trace admin activity for incident response','Visibility into script abuse and lateral movement');
  'AppLocker'         = @('Constrain code execution to trusted sources','Reduce malware and LOLBIN risk');
  'Firewall'          = @('Default-deny inbound; strict egress','Reduce attack surface and exfiltration');
  'TLS'               = @('Eliminate legacy protocol/cipher risk','Prevent downgrade and weak crypto exposure');
  'Account Policy'    = @('Thwart brute force and password spraying','Lockout prevents credential stuffing success');
  'WinRM'             = @('Harden remote management channel','Prevent cleartext/weak listeners');
  'TPM/SecureBoot'    = @('Root-of-trust and boot integrity','Defend against bootkits and credential theft');
  'Exploit Protection'= @('Memory exploit mitigations by default','Mitigate ROP and memory corruption exploits');
  'Defender'          = @('Protect and monitor endpoint threats','Maintain sensor integrity and coverage');
  'WDAC'              = @('Whitelist-only code integrity','Block untrusted binaries and drivers');
  'Accounts'          = @('Least privilege on endpoints','Reduce blast radius of compromise');
  'Services'          = @('Disable risky legacy services','Shrink lateral movement surface');
  'Browser'           = @('Block phishing and drive-by downloads','Reduce user-originated compromise');
  'Telemetry'         = @('Limit outbound diagnostic data','Reduce PII exposure');
}

# Prereq hints by Id (fallback if not present already)
$PrereqMap = @{
  'NEW-TPM-01' = 'TPM present & ready';
  'NEW-TPM-02' = 'UEFI + SecureBoot capability';
  'NEW-WRM-01' = 'WinRM installed';
  'NEW-AL-01'  = 'AppLocker feature available (Ent/Edu)';
  'NEW-WDAC-01'= 'WDAC feature available';
  'NEW-DEF-01' = 'Windows Defender present';
  'NEW-TLS-01' = 'Schannel Server path present';
  'NEW-FW-01'  = 'Windows Firewall service present';
}

# Normalize into a projection that includes Prereq + CompliantBool + Why/Purpose
$Results = @(
  foreach ($r in $Results) {
    # Clone into a PSCustomObject to stabilize column order
    $obj = [PSCustomObject]@{
      Id        = $r.Id
      Priority  = $r.Priority
      Area      = $r.Area
    ...
    # Ensure Prereq
    $pr = $null
    if ($r.PSObject.Properties.Name -contains 'Prereq') { $pr = $r.Prereq }
    if (-not $pr -and $PrereqMap.ContainsKey($r.Id)) { $pr = $PrereqMap[$r.Id] }
    if (-not $pr) { $pr = 'None' }
    Add-Member -InputObject $obj -NotePropertyName 'Prereq' -NotePropertyValue $pr

    # Add CompliantBool
    $cb = $null
    if ($null -ne $r.Compliant) {
      $val = $r.Compliant.ToString().ToUpper()
      if ($val -eq 'PASS') { $cb = $true }
      elseif ($val -eq 'FAIL') { $cb = $false }
      elseif ($val -eq 'TRUE') { $cb = $true }
      elseif ($val -eq 'FALSE') { $cb = $false }
    }
    if ($null -eq $cb) { $cb = $false } # default conservative
    Add-Member -InputObject $obj -NotePropertyName 'CompliantBool' -NotePropertyValue $cb

    # Backfill Why/Purpose if missing and Area is known
    if ((-not $obj.Why) -and $WhyPurposeByArea.ContainsKey($obj.Area)) { $obj.Why = $WhyPurposeByArea[$obj.Area][0] }
    if ((-not $obj.Purpose) -and $WhyPurposeByArea.ContainsKey($obj.Area)) { $obj.Purpose = $WhyPurposeByArea[$obj.Area][1] }

    $obj
  }
)
# ======= end v3.4f export normalization =======



# ======= v3.4g explicit export projection =======
# Force explicit shape so exporters include new fields
$Results = @(
  foreach ($r in $Results) {
    $pr = $null
    if ($r.PSObject.Properties.Name -contains 'Prereq') { $pr = $r.Prereq } else { $pr = 'None' }
    $cb = $false
    if ($null -ne $r.Compliant) {
      $val = $r.Compliant.ToString().ToUpper()
      if ($val -eq 'PASS' -or $val -eq 'TRUE') { $cb = $true }
    }
    [PSCustomObject]@{
      Id            = $r.Id
      Priority      = $r.Priority
...
# ======= end v3.4g explicit export projection =======


Normalize-Results -Results ([ref]$Results)

Normalize-Results -Results ([ref]$Results)
ConvertTo-Json -Depth 6 | Out-File -Encoding UTF8 $JsonPath
  $Unified | Export-Csv -NoTypeInformation -Encoding UTF8 $CsvPath
  $summary = $Unified | Group-Object Category | ForEach-Object {
    $cat=$_.Name; $total=$_.Group.Count; $failed=($_.Group | Where-Object {$_.Passed -eq $false}).Count
    $highFail=($_.Group | Where-Object {$_.Severity -match 'High' -and -not $_.Passed}).Count
    $passrate=if($total -gt 0){[math]::Round((($total-$failed)/$total)*100,2)}else{0}
    [PSCustomObject]@{Category=$cat;Total=$total;Failed=$failed;HighFail=$highFail;PassRate="$passrate%"}
  }
  $SummaryPath = Join-Path -Path (Split-Path -Parent $CsvPath) -ChildPath "Win_Hardening_Audit_v3_summary.csv"
  $summary | Export-Csv -NoTypeInformation -Encoding UTF8 $SummaryPath
}

# Force our export to run LAST, even if legacy export runs earlier
Register-EngineEvent -SourceIdentifier PowerShell.Exiting -Action { __Normalize-And-Export } | Out-Null
# =====================================================================================
...
# ===== Injected NEW checks (executed early in audit) =====
# ======== Proposed Missing Checks (Stubs) ========
# Style: imperative, no trailing period
try {
  $transcript = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\Transcription' -Name 'EnableTranscripting' -ErrorAction SilentlyContinue
  Add-Result -Id 'NEW-PS-01' -Priority 'High' -Area 'PowerShell' -Check 'Enable PowerShell transcription logging' -Expected '1' -Actual $($transcript.EnableTranscripting) -Compliant (Test-ValueEq $transcript.EnableTranscripting 1)
} catch { Add-Error 'NEW-PS-01' $_ }
try {
  ...
  Add-Result -Id 'NEW-PS-02' -Priority 'High' -Area 'PowerShell' -Check 'Enable PowerShell module logging' -Expected '1' -Actual $($moduleLog.EnableModuleLogging) -Compliant (Test-ValueEq $moduleLog.EnableModuleLogging 1)
} catch { Add-Error 'NEW-PS-02' $_ }
try {
  $applocker = Get-AppLockerPolicy -Effective -ErrorAction SilentlyContinue
  $hasRules = ($applocker.RuleCollections | Where-Object { $_.Rules.Count -gt 0 }).Count -gt 0
  Add-Result -Id 'NEW-AL-01' -Priority 'High' -Area 'AppLocker' -Check 'Enforce AppLocker rules for EXE Script MSI DLL' -Expected 'Present/Enforced' -Actual $(if ($hasRules) {'Present'} else {'Missing'}) -Compliant $hasRules
} catch { Add-Error 'NEW-AL-01' $_ }
try {
  $fw = (Get-NetFirewallProfile | Select-Object Name, Enabled, DefaultInboundAction, DefaultOutboundAction)
  $allEnabled = -not ($fw | Where-Object { -not $_.Enabled }).Count
  Add-Result -Id 'NEW-FW-01' -Priority 'High' -Area 'Firewall' -Check 'Enable Windows Firewall on all profiles' -Expected 'Enabled' -Actual $allEnabled -Compliant $allEnabled
} catch { Add-Error 'NEW-FW-01' $_ }
try {
  ...
  Add-Result -Id 'NEW-TLS-01' -Priority 'High' -Area 'TLS' -Check 'Disable TLS 1.0 Server' -Expected '0' -Actual $($tls10.Enabled) -Compliant (Test-ValueEq $tls10.Enabled 0)
} catch { Add-Error 'NEW-TLS-01' $_ }
try {
  $null = (secedit /export /cfg $env:TEMP\secpol.cfg) > $null; $cfg = Get-Content $env:TEMP\secpol.cfg
  $threshold = ($cfg | Where-Object { $_ -match '^LockoutBadCount\s*=\s*(\d+)$' } | ForEach-Object { [int]($matches[1]) } | Select-Object -First 1)
  Add-Result -Id 'NEW-AC-01' -Priority 'High' -Area 'Account Policy' -Check 'Enforce account lockout threshold >=10' -Expected '>=10' -Actual $threshold -Compliant ($threshold -ge 10)
} catch { Add-Error 'NEW-AC-01' $_ }
try {
  $svc = Get-Service WinRM -ErrorAction SilentlyContinue
  $https = (winrm enumerate winrm/config/listener | Select-String -SimpleMatch 'HTTPS') -ne $null
} catch { Add-Error 'NEW-WRM-01' $_ }
try {
  $wrmOk = $false
  $httpsOnly = $false
  $tw = Test-WSMan -ErrorAction SilentlyContinue
  if ($tw) {
    # Check listeners via WSMan drive; avoid 'winrm' command
    $listeners = @(Get-ChildItem WSMan:\localhost\Listener -ErrorAction SilentlyContinue)
    $protoSet = @($listeners | ForEach-Object { $_.Keys['Transport'] }) -join ';'
    $httpsOnly = ($protoSet -ne '') -and ($protoSet -notmatch '(?i)HTTP') -and ($protoSet -match '(?i)HTTPS')
    $wrmOk = $true
  }
  Add-Result -Id 'NEW-WRM-01' -Priority 'Medium' -Area 'WinRM' -Check 'Restrict WinRM to HTTPS listener' -Expected 'HTTPS only' -Actual $(if($wrmOk){ if($httpsOnly){'HTTPS'} else {'HTTP/None'} } else {'WinRM not configured'}) -Compliant ($wrmOk -and $httpsOnly)
} catch { Add-Error 'NEW-WRM-01' $_ }

try {
  $tpm = Get-Tpm
  Add-Result -Id 'NEW-TPM-01' -Priority 'High' -Area 'TPM/SecureBoot' -Check 'Enable and own TPM' -Expected 'Present/Owned' -Actual $($tpm.TpmPresent -and $tpm.TpmReady) -Compliant ($tpm.TpmPresent -and $tpm.TpmReady)
} catch { Add-Error 'NEW-TPM-01' $_ }
try {
  $sb = Confirm-SecureBootUEFI -ErrorAction SilentlyContinue
  Add-Result -Id 'NEW-TPM-02' -Priority 'High' -Area 'TPM/SecureBoot' -Check 'Enable Secure Boot' -Expected 'True' -Actual $sb -Compliant (Test-ValueEq $sb $true)
} catch { Add-Error 'NEW-TPM-02' $_ }
try {
  $sysCfg = Get-ProcessMitigation -System
  $aslr = $sysCfg.ASLR.ForceRelocateImages
  Add-Result -Id 'NEW-EP-01' -Priority 'High' -Area 'Exploit Protection' -Check 'Enforce system-wide ASLR' -Expected 'ON' -Actual $aslr -Compliant ($aslr -match 'ON')
} catch { Add-Error 'NEW-EP-01' $_ }
try {
  $tp = Get-MpPreference | Select-Object -ExpandProperty DisableTamperProtection -ErrorAction SilentlyContinue
  Add-Result -Id 'NEW-DEF-01' -Priority 'High' -Area 'Defender' -Check 'Enable Defender Tamper Protection' -Expected '0' -Actual $tp -Compliant (Test-ValueEq $tp 0)
} catch { Add-Error 'NEW-DEF-01' $_ }
try {
  $wd = Get-CimInstance -Namespace 'root\Microsoft\Windows\DeviceGuard' -ClassName 'Win32_DeviceGuard'
  $umciOn = ($wd.UserModeCodeIntegrityPolicyEnforced -eq 1)
  Add-Result -Id 'NEW-WDAC-01' -Priority 'High' -Area 'WDAC' -Check 'Enforce WDAC UMCI policy' -Expected 'Enforced' -Actual $umciOn -Compliant $umciOn
} catch { Add-Error 'NEW-WDAC-01' $_ }
try {
  $admins = (Get-LocalGroupMember -Group 'Administrators' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name) -join ';'
  Add-Result -Id 'NEW-LA-01' -Priority 'High' -Area 'Accounts' -Check 'Review local Administrators group membership' -Expected 'Least privilege' -Actual ($admins -join ';') -Compliant ($admins -notmatch '.*\bEveryone\b|.*\bDomain Users\b')
} catch { Add-Error 'NEW-LA-01' $_ }
try {
  $svc = Get-Service RemoteRegistry -ErrorAction SilentlyContinue
  $ok = $svc -and ($svc.StartType -eq 'Disabled')
  Add-Result -Id 'NEW-SVC-01' -Priority 'Medium' -Area 'Services' -Check 'Disable Remote Registry service' -Expected 'Disabled' -Actual $(if ($svc) {$svc.StartType} else {'Not Present'}) -Compliant $ok
} catch { Add-Error 'NEW-SVC-01' $_ }
try {
  $smartscreen = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Edge' -Name 'SmartScreenEnabled' -ErrorAction SilentlyContinue
  Add-Result -Id 'NEW-BRW-01' -Priority 'Medium' -Area 'Browser' -Check 'Enable Edge SmartScreen' -Expected '1' -Actual $($smartscreen.SmartScreenEnabled) -Compliant (Test-ValueEq $smartscreen.SmartScreenEnabled 1)
} catch { Add-Error 'NEW-BRW-01' $_ }
try {
  $diag = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection' -Name 'AllowTelemetry' -ErrorAction SilentlyContinue
  Add-Result -Id 'NEW-TEL-01' -Priority 'Low' -Area 'Telemetry' -Check 'Restrict diagnostic data collection <=1' -Expected '<=1' -Actual $($diag.AllowTelemetry) -Compliant ( ($diag -ne $null) -and ([int]$diag.AllowTelemetry -le 1) )
} catch { Add-Error 'NEW-TEL-01' $_ }
# ======== End Proposed Missing Checks ========

# ===== End Injected NEW checks =====
Add-Result -Id 'ENV-01' -Priority 'Info' -Area 'OS' -Check 'Windows Version/Build' -Expected 'Win10/11 supported release' -Actual $actualEnv -Compliant $true
  ...
  Add-Result -Id 'ENV-01-R1' -Priority 'Info' -Area 'OS' -Check 'Windows Version/Build' -Expected 'Win10/11 supported release' -Actual 'OS query failed' -Compliant $false
}
...
} catch { Add-Result -Id 'HP-11-R1' -Priority 'High' -Area 'Defender' -Check 'Network Protection' -Expected '1 (Block)' -Actual 'Not configured' -Compliant $false }
...
} catch { Add-Result -Id 'HP-12-R1' -Priority 'High' -Area 'Ransomware' -Check 'Controlled Folder Access' -Expected '1 (Block)' -Actual 'Get-MpPreference failed' -Compliant $false }
...
} catch { Add-Result -Id 'HP-13a-R1' -Priority 'High' -Area 'Boot Security' -Check 'Secure Boot' -Expected 'Enabled' -Actual 'Unsupported/No UEFI' -Compliant $false }
...
} catch { Add-Result -Id 'HP-13b-R1' -Priority 'High' -Area 'Boot Security' -Check 'TPM Ready' -Expected 'Present & Ready' -Actual 'Get-Tpm failed' -Compliant $false }
...
} catch { Add-Result -Id 'HP-14-R1' -Priority 'High' -Area 'BitLocker' -Check 'OS Drive Encryption' -Expected 'FullyEncrypted XTS-AES' -Actual 'BitLocker not available' -Compliant $false }
...
} catch { Add-Result -Id 'MP-12e-R1' -Priority 'Medium' -Area 'RDP/Firewall' -Check 'Inbound rules disabled when RDP off' -Expected 'No enabled allow rules if RDP disabled' -Actual 'Rule query failed' -Compliant $false }
...
} catch { Add-Result -Id 'MP-27a-R1' -Priority 'Medium' -Area 'Accounts' -Check 'Guest account disabled' -Expected 'Disabled' -Actual 'Guest missing/handled' -Compliant $true }
try {
...
} catch { Add-Result -Id 'MP-27b-R1' -Priority 'Medium' -Area 'Accounts' -Check 'Administrator account renamed' -Expected 'Not "Administrator"' -Actual 'Query failed' -Compliant $false }
...
} catch { Add-Result -Id 'MP-44-R1' -Priority 'Medium' -Area 'SMB Client' -Check 'Client configuration' -Expected 'Accessible' -Actual 'Get-SmbClientConfiguration failed' -Compliant $false }
...
} catch { Add-Result -Id 'MP-46-R1' -Priority 'Medium' -Area 'SMB' -Check 'NullSessionPipes' -Expected '0 entries' -Actual 'Query failed' -Compliant $false }
...
# =====================[ BEGIN: Additions & GPO Coverage ]=====================

# small helpers
function Get-GpoReg { param(
  [Parameter(Mandatory)][string]$Path,
  [Parameter(Mandatory)][string]$Name,
  [ValidateSet('HKLM','HKCU')]$Hive='HKLM'
) try { (Get-ItemProperty -LiteralPath "$($Hive):\$Path" -ErrorAction Stop).$Name } catch { $null } }

function Add-Why { param([string]$Why,[string]$Purpose)
  if ($Results.Count -gt 0) {
    $Results[-1] | Add-Member -NotePropertyName Why -NotePropertyValue $Why -Force
    $Results[-1] | Add-Member -NotePropertyName Purpose -NotePropertyValue $Purpose -Force
  }
}

# ---------- 1) “Add these to script and optimize it” list ----------

# A1. ASR Rules – annotate existing ASR rows
foreach ($r in $Results | Where-Object { $_.Id -like 'HP-10-*' }) {
  $r | Add-Member Why 'Stops common Office/WMI/LSASS abuse paths' -Force
  $r | Add-Member Purpose 'Reduce malware foothold and LOLBins by default-deny at process creation time' -Force
}

# A2. TLS Protocols (MP-24) – annotate
foreach ($r in $Results | Where-Object { $_.Id -like 'MP-24-*' }) {
  $r | Add-Member Why 'Eliminate legacy TLS downgrade and weak cipher exposure' -Force
  $r | Add-Member Purpose 'Ensure modern crypto-only channels (TLS 1.2/1.3)' -Force
}

# A3. PowerShell v2 (MP-23) – annotate
foreach ($r in $Results | Where-Object { $_.Id -eq 'MP-23' }) {
  $r | Add-Member Why 'PSv2 lacks modern logging & security controls' -Force
  $r | Add-Member Purpose 'Force PowerShell versions that support auditing and AMSI' -Force
}

# A4. RDP Redirection – audio & printer
$tsPol = 'SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services'
$rdpAudioMode = Get-GpoReg -Path $tsPol -Name 'AudioRedirectionMode'        # 2=disable
$rdpNoAudioCap = Get-GpoReg -Path $tsPol -Name 'fDisableAudioCapture'        # 1=disable
$rdpDefPrinter = Get-GpoReg -Path $tsPol -Name 'RedirectOnlyDefaultClientPrinter' # 1=only default
Add-Result -Id 'MP-12f' -Priority 'Medium' -Area 'RDP' -Check 'Disable audio redirection' -Expected 'AudioRedirectionMode=2' -Actual $rdpAudioMode -Compliant (Test-ValueEq $rdpAudioMode 2)
Add-Why 'Stops audio channel data exfil/callbacks' 'Shrink RDP attack surface and data egress paths'

Add-Result -Id 'MP-12g' -Priority 'Medium' -Area 'RDP' -Check 'Disable audio capture' -Expected 'fDisableAudioCapture=1' -Actual $rdpNoAudioCap -Compliant (Test-ValueEq $rdpNoAudioCap 1)
Add-Why 'Prevents mic capture over RDP' 'Reduce privacy and info-leak risk via RDP'

Add-Result -Id 'MP-12h' -Priority 'Medium' -Area 'RDP' -Check 'Restrict printer redirection' -Expected 'RedirectOnlyDefaultClientPrinter=1' -Actual $rdpDefPrinter -Compliant (Test-ValueEq $rdpDefPrinter 1)
Add-Why 'Minimize device injection surface' 'Constrain redirected device scope to least necessary'

# A5. BitLocker – require TPM+PIN
try {
  $blOS = Get-BitLockerVolume -MountPoint $env:SystemDrive -ErrorAction Stop
  $hasTpmPin = $false
  foreach ($kp in $blOS.KeyProtector) {
    if ($kp.KeyProtectorType -match 'TpmPin|TpmAndPin') { $hasTpmPin = $true; break }
  }
  Add-Result -Id 'HP-14b' -Priority 'High' -Area 'BitLocker' -Check 'TPM+PIN protector' -Expected 'Present' -Actual ("HasTpmPin={0}" -f $hasTpmPin) -Compliant $hasTpmPin
  Add-Why 'Mitigate stolen device + boot bypass' 'Bind decryption to something you have (TPM) + know (PIN)'
} catch {
  Add-Result -Id 'HP-14b-R1' -Priority 'High' -Area 'BitLocker' -Check 'TPM+PIN protector' -Expected 'Present' -Actual 'Query failed' -Compliant $false
  Add-Why 'Mitigate stolen device + boot bypass' 'Bind decryption to something you have (TPM) + know (PIN)'
}

# A6. Account Policy – history/age/length/complexity
$net = (net accounts 2>&1 | Out-String)
function _grabInt { param([string]$s,[string]$rx) if ($s -match $rx) { [int]$matches[1] } else { -1 } }
$hist   = _grabInt $net 'Minimum password age\s+(\d+)\s+days'
$maxage = _grabInt $net 'Maximum password age\s+(\d+)\s+days'
$minage = _grabInt $net 'Minimum password age\s+(\d+)\s+days'
$minlen = _grabInt $net 'Minimum password length\s+(\d+)'
$lockth = _grabInt $net 'Lockout threshold:\s+(\d+)'
$pwdComplex = Get-GpoReg -Path 'SYSTEM\CurrentControlSet\Control\Lsa' -Name 'PasswordComplexity'
$revEnc     = Get-GpoReg -Path 'SYSTEM\CurrentControlSet\Control\Lsa' -Name 'ClearTextPassword'
Add-Result -Id 'GPO-ACCT-01' -Priority 'Medium' -Area 'Account Policy' -Check 'Password history' -Expected '>= 24' -Actual $hist -Compliant ($hist -ge 24); Add-Why 'Prevents rotation reuse' 'Increase entropy across cycles'
Add-Result -Id 'GPO-ACCT-02' -Priority 'Medium' -Area 'Account Policy' -Check 'Maximum password age' -Expected '<= 60 days' -Actual ("{0} days" -f $maxage) -Compliant ($maxage -le 60 -and $maxage -gt 0); Add-Why 'Limit stale secrets' 'Force periodic credential turnover'
Add-Result -Id 'GPO-ACCT-03' -Priority 'Medium' -Area 'Account Policy' -Check 'Minimum password age' -Expected '>= 1 day' -Actual ("{0} days" -f $minage) -Compliant ($minage -ge 1); Add-Why 'Block rapid reuse' 'Stop users cycling to old passwords immediately'
Add-Result -Id 'GPO-ACCT-04' -Priority 'Medium' -Area 'Account Policy' -Check 'Minimum password length' -Expected '>= 14' -Actual $minlen -Compliant ($minlen -ge 14); Add-Why 'Raise brute-force cost' 'Increase search space of guesses'
Add-Result -Id 'GPO-ACCT-05' -Priority 'Medium' -Area 'Account Policy' -Check 'Password complexity' -Expected 'PasswordComplexity=1' -Actual $pwdComplex -Compliant (Test-ValueEq $pwdComplex 1); Add-Why 'Stop trivial passwords' 'Require classes/mix as per policy'
Add-Result -Id 'GPO-ACCT-06' -Priority 'Medium' -Area 'Account Policy' -Check 'Reversible encryption' -Expected 'Disabled (0)' -Actual $revEnc -Compliant ( ($revEnc -eq $null) -or (Test-ValueEq $revEnc 0) ); Add-Why 'Avoid storing password equivalents' 'Reduce credential exposure in SAM/LSA'
Add-Result -Id 'GPO-ACCT-07' -Priority 'Medium' -Area 'Account Policy' -Check 'Lockout threshold' -Expected '<= 5' -Actual $lockth -Compliant ($lockth -le 5 -and $lockth -ge 0); Add-Why 'Throttle online guessing' 'Force lockout on repeated failures'

# A7. Tag existing Auditing rows with WHY/Purpose
foreach ($r in $Results | Where-Object { $_.Area -eq 'Auditing' }) {
  $r | Add-Member Why 'Forensic visibility for auth, policy, object, and process events' -Force
  $r | Add-Member Purpose 'Support incident response, UEBA, and compliance' -Force
}

# A8. EnableLUA
$enableLUA = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'EnableLUA'
Add-Result -Id 'GPO-UAC-01' -Priority 'High' -Area 'UAC' -Check 'EnableLUA' -Expected '1' -Actual $enableLUA -Compliant (Test-ValueEq $enableLUA 1); Add-Why 'Mandatory UAC boundary' 'Prevent silent elevation & token abuse'

# A9. UAC Secure Desktop
$securePrompt = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'PromptOnSecureDesktop'
Add-Result -Id 'GPO-UAC-02' -Priority 'High' -Area 'UAC' -Check 'Prompt on Secure Desktop' -Expected '1' -Actual $securePrompt -Compliant (Test-ValueEq $securePrompt 1); Add-Why 'Thwarts clickjacking of consent UI' 'Isolate elevation prompts from user desktop'

# A10. Kerberos AES-only
$kerbTypes = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Kerberos\Parameters' -Name 'SupportedEncryptionTypes'
$krbOk = ($null -ne $kerbTypes) -and ( ([int]$kerbTypes -band 0x18) -eq 0x18) -and (([int]$kerbTypes -band 0x04) -eq 0)
Add-Result -Id 'GPO-KRB-01' -Priority 'High' -Area 'Kerberos' -Check 'Restrict to AES' -Expected 'SupportedEncryptionTypes=0x18 (no RC4/MD5)' -Actual $kerbTypes -Compliant $krbOk; Add-Why 'Defend against RC4 downgrade & weak crypto' 'Standardize on AES for tickets & preauth'

# A11. Minimum SMB dialect (effective)
$smcOK=$false; $smsOK=$false
try { $cli=Get-SmbClientConfiguration -ErrorAction Stop; $smcOK = (-not $cli.EnableInsecureGuestLogons) -and $cli.EnableSecuritySignature } catch {}
try { $srv=Get-SmbServerConfiguration -ErrorAction Stop; $smsOK = (-not $srv.EnableSMB1Protocol) -and $srv.EnableSMB2Protocol } catch {}
Add-Result -Id 'GPO-SMB-01' -Priority 'High' -Area 'SMB' -Check 'Minimum dialect >= 2.x (effective)' -Expected 'SMB1 off + SMB2 on' -Actual ("ClientOK={0};ServerOK={1}" -f $smcOK,$smsOK) -Compliant ($smcOK -and $smsOK); Add-Why 'Remove legacy SMBv1 exploits (WannaCry class)' 'Assure modern SMB dialect negotiation only'

# A12. 4688 Command-line Auditing
$cmdLine = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Audit' -Name 'ProcessCreationIncludeCmdLine_Enabled'
Add-Result -Id 'GPO-AUD-4688' -Priority 'Medium' -Area 'Auditing' -Check 'Include command line in 4688' -Expected '1' -Actual $cmdLine -Compliant (Test-ValueEq $cmdLine 1); Add-Why 'Hunt LOLBins & living-off-the-land' 'Capture full process context for detections'

# A13. Edge Extension Control
$edgeBase = 'SOFTWARE\Policies\Microsoft\Edge'
$blockList = Get-GpoReg -Path $edgeBase -Name 'ExtensionInstallBlocklist'
$allowList = Get-GpoReg -Path $edgeBase -Name 'ExtensionInstallAllowlist'
$blockedAll = $false
if ($blockList -is [array]) { $blockedAll = ($blockList -contains '*') } elseif ($blockList -is [string]) { $blockedAll = ($blockList -eq '*') }
Add-Result -Id 'GPO-EDGE-EXT' -Priority 'Medium' -Area 'Edge' -Check 'Block unapproved extensions' -Expected 'Blocklist="*" + allowlist' -Actual ("BlockAll={0};Allowlist={1}" -f $blockedAll, $(if($allowList){'Yes'}else{'No'})) -Compliant ($blockedAll -and ($null -ne $allowList)); Add-Why 'Cut malware/adware add-ons' 'Default-deny extensions, permit only approved IDs'

# A14. Windows Update Deadlines
$ux='SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\UX\Settings'
$featDeadline = Get-GpoReg -Path $ux -Name 'DeferFeatureUpdatesPeriodInDays'
$qualDeadline = Get-GpoReg -Path $ux -Name 'DeferQualityUpdatesPeriodInDays'
$deadFeature  = Get-GpoReg -Path $ux -Name 'FeatureUpdatesDeferralInDays'
$deadQuality  = Get-GpoReg -Path $ux -Name 'QualityUpdatesDeferralInDays'
$deadlineOK = ($deadFeature -ge 0 -or $featDeadline -ge 0) -and ($deadQuality -ge 0 -or $qualDeadline -ge 0)
Add-Result -Id 'GPO-WU-01' -Priority 'Medium' -Area 'Windows Update' -Check 'Deadlines configured' -Expected 'Feature & Quality deadlines set' -Actual ("F={0}/{1}; Q={2}/{3}" -f $deadFeature,$featDeadline,$deadQuality,$qualDeadline) -Compliant $deadlineOK; Add-Why 'Kill “forever pending” patch debt' 'Enforce time-bound installation behavior'

# A15. Device Installation restrictions
$devBase='SOFTWARE\Policies\Microsoft\Windows\DeviceInstall\Restrictions'
$denyRetro = Get-GpoReg -Path $devBase -Name 'DenyDeviceIDsRetroactive'
$denyAny   = Get-GpoReg -Path $devBase -Name 'DenyUnspecified'
Add-Result -Id 'GPO-DEV-01' -Priority 'Medium' -Area 'Device Control' -Check 'Device install restrictions' -Expected 'DenyUnspecified=1 + Retroactive=1' -Actual ("DenyUnspecified={0};Retro={1}" -f $denyAny,$denyRetro) -Compliant ( (Test-ValueEq $denyAny 1) -and (Test-ValueEq $denyRetro 1) ); Add-Why 'Stop rogue peripherals' 'Block unknown PnP device classes by default'

# A16. VBS Runtime Verification
try {
  $dg = Get-CimInstance -ClassName Win32_DeviceGuard -ErrorAction Stop
  $vbs = $dg.VirtualizationBasedSecurityStatus -eq 1
  $svc = $dg.SecurityServicesRunning
  $cg  = ($svc -contains 1)      # 1 = Credential Guard
  $hvci= ($svc -contains 2)      # 2 = HVCI
  Add-Result -Id 'HP-06c' -Priority 'High' -Area 'VBS' -Check 'VBS runtime status' -Expected 'On (CG/HVCI running)' -Actual ("VBS={0};CG={1};HVCI={2}" -f $vbs,$cg,$hvci) -Compliant ($vbs -and ($cg -or $hvci)); Add-Why 'Validate not just configured but running' 'Close the “configured but not active” gap'
} catch {
  Add-Result -Id 'HP-06c-R1' -Priority 'High' -Area 'VBS' -Check 'VBS runtime status' -Expected 'On (CG/HVCI running)' -Actual 'Query failed' -Compliant $false; Add-Why 'Validate not just configured but running' 'Close the “configured but not active” gap'
}

# A17. Windows Firewall – disable local rule merge
try {
  $pfs = Get-NetFirewallProfile -ErrorAction Stop
  foreach ($p in $pfs) {
    $ok = ($p.AllowLocalPolicyMerge -eq 'False' -or $p.AllowLocalPolicyMerge -eq $false)
    Add-Result -Id ("GPO-FW-LOCALMERGE-{0}" -f $p.Name) -Priority 'Medium' -Area 'Firewall' -Check ("Disable local rule merge ({0})" -f $p.Name) -Expected 'AllowLocalPolicyMerge=False' -Actual ("{0}" -f $p.AllowLocalPolicyMerge) -Compliant $ok
    Add-Why 'Ensure GPO enforcement beats local overrides' 'Prevent users/apps from sneaking in allow rules'
  }
} catch {
  Add-Result -Id 'GPO-FW-LOCALMERGE' -Priority 'Medium' -Area 'Firewall' -Check 'Disable local rule merge' -Expected 'AllowLocalPolicyMerge=False' -Actual 'Get-NetFirewallProfile failed' -Compliant $false
  Add-Why 'Ensure GPO enforcement beats local overrides' 'Prevent users/apps from sneaking in allow rules'
}

# ---------- Security Options and extras ----------

# Legal notice (banner)
$legalCap = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'legalnoticecaption'
$legalTxt = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'legalnoticetext'
Add-Result -Id 'GPO-SEC-LEGAL' -Priority 'Low' -Area 'Security Options' -Check 'Logon legal banner' -Expected 'Caption/Text present' -Actual ("CapEmpty={0};TxtLen={1}" -f [string]::IsNullOrWhiteSpace($legalCap), ($legalTxt | Out-String).Length) -Compliant ((-not [string]::IsNullOrWhiteSpace($legalCap)) -and (-not [string]::IsNullOrWhiteSpace($legalTxt))); Add-Why 'Notify & deter; support prosecution' 'Show authorized use notice at logon'

# Disallow shutdown without logon
$noShut = Get-GpoReg -Path 'SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name 'ShutdownWithoutLogon'
Add-Result -Id 'GPO-SEC-SD' -Priority 'Medium' -Area 'Security Options' -Check 'Disallow shutdown without logon' -Expected 'ShutdownWithoutLogon=0' -Actual $noShut -Compliant (Test-ValueEq $noShut 0); Add-Why 'Stop kiosk-style tampering' 'Require auth before power control'

# User Rights extension
$cfg = Join-Path $env:TEMP ('secpol_ex_{0}.inf' -f ([guid]::NewGuid())); secedit /export /cfg $cfg | Out-Null
$ulines = @(); try { $ulines = Get-Content $cfg -ErrorAction SilentlyContinue } catch {}
Remove-Item $cfg -Force -ErrorAction SilentlyContinue
function _ur { param([string]$n) ($ulines | Where-Object { $_ -match "^$n\s*=" } | Select-Object -First 1) }
function _has($line,$sid){ if ($null -eq $line) { $false } else { $line -match $sid } }

$denyLocal = _ur 'SeDenyInteractiveLogonRight'
$denyRDS   = _ur 'SeDenyRemoteInteractiveLogonRight'
$allowLocal= _ur 'SeInteractiveLogonRight'
$allowRDS  = _ur 'SeRemoteInteractiveLogonRight'

Add-Result -Id 'GPO-UR-01' -Priority 'Medium' -Area 'User Rights' -Check 'Deny log on locally → Guests' -Expected 'Guests (S-1-5-32-546)' -Actual $denyLocal -Compliant (_has $denyLocal 'S-1-5-32-546'); Add-Why 'Remove human footholds' 'Guests should never get console sessions'
Add-Result -Id 'GPO-UR-02' -Priority 'Medium' -Area 'User Rights' -Check 'Deny RDP logon → Guests & Local' -Expected 'Guests + S-1-5-113/S-1-5-114' -Actual $denyRDS -Compliant ( (_has $denyRDS 'S-1-5-32-546') -and (_has $denyRDS 'S-1-5-113') -and (_has $denyRDS 'S-1-5-114') ); Add-Why 'Stop lateral from locals' 'Force domain-auth only paths for RDP'
Add-Result -Id 'GPO-UR-03' -Priority 'Low' -Area 'User Rights' -Check 'Allow log on locally → Administrators only' -Expected 'Administrators' -Actual $allowLocal -Compliant (_has $allowLocal 'S-1-5-32-544'); Add-Why 'Least privilege at console' 'Limit who can access interactive desktop'
Add-Result -Id 'GPO-UR-04' -Priority 'Low' -Area 'User Rights' -Check 'Allow log on via RDS → Admins only' -Expected 'Administrators' -Actual $allowRDS -Compliant (_has $allowRDS 'S-1-5-32-544'); Add-Why 'Harden entry points' 'Only admins can obtain RDP sessions by policy'

# Admin Templates → System: annotate your WER check
foreach ($r in $Results | Where-Object { $_.Id -eq 'LP-05' }) {
# removed direct property set on result object
}

# RDS idle timeout
$idle = Get-GpoReg -Path $tsPol -Name 'IdleTimeout'
Add-Result -Id 'GPO-RDP-IDLE' -Priority 'Low' -Area 'RDP' -Check 'Idle timeout' -Expected '>0 (enforced)' -Actual $idle -Compliant ( ($idle -ne $null) -and ([int64]$idle -gt 0) ); Add-Why 'Reduce orphaned sessions' 'Shrink window for session hijack'

# BitLocker policy refinements
$encOSpol = Get-GpoReg -Path 'SOFTWARE\Policies\Microsoft\FVE' -Name 'EncryptionMethodWithXtsOs'
Add-Result -Id 'GPO-BL-01' -Priority 'High' -Area 'BitLocker' -Check 'XTS-AES 256 for OS' -Expected '6 (XTS-256)' -Actual $encOSpol -Compliant (Test-ValueEq $encOSpol 6); Add-Why 'Raise brute-force cost at rest' 'Standardize strong cipher for all drives'

# Windows Update: disable Pause (UX hide)
$pause = Get-GpoReg -Path 'SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate' -Name 'SetDisableUXWUAccess'
Add-Result -Id 'GPO-WU-02' -Priority 'Low' -Area 'Windows Update' -Check 'Disable user WU UX (no pause)' -Expected 'SetDisableUXWUAccess=1' -Actual $pause -Compliant (Test-ValueEq $pause 1); Add-Why 'Remove user deferrals' 'Centralize patch cadence'

# Windows Event Forwarding presence
$wefSvc = Get-Service -Name 'Wecsvc' -ErrorAction SilentlyContinue
if ($wefSvc) {
  Add-Result -Id 'GPO-WEF-01' -Priority 'Low' -Area 'Logging/WEF' -Check 'Windows Event Collector' -Expected 'Running/Auto' -Actual ("{0}/{1}" -f $wefSvc.Status,$wefSvc.StartType) -Compliant ( ($wefSvc.Status -eq 'Running') -and ($wefSvc.StartType -in 'Automatic','AutomaticDelayedStart') ); Add-Why 'Centralize telemetry' 'Enable source-initiated subscriptions'
}

# Misc Enterprise Controls: Nearby Sharing off
$nearby = Get-GpoReg -Path 'SOFTWARE\Policies\Microsoft\Windows\System' -Name 'EnableNearShare'
Add-Result -Id 'GPO-MISC-NS' -Priority 'Low' -Area 'System' -Check 'Disable Nearby Sharing' -Expected 'EnableNearShare=0' -Actual $nearby -Compliant (Test-ValueEq $nearby 0); Add-Why 'Reduce casual data exfil' 'Limit ad-hoc radio sharing features'

# =====================[ END: Additions & GPO Coverage ]=====================
...
# =======================================================================
$Results | Select-Object Id, Priority, Area, Check, Expected, Actual, Compliant, Why, Purpose, Prereq, CompliantBool | Export-Csv -NoTypeInformation -Encoding UTF8 $CsvPath
...
# ======= Additional Baseline Checks (appended by assistant) =======
function Invoke-AdditionalBaselineChecks {
  Write-Host "Running additional baseline checks..." -ForegroundColor Cyan

  # --- CredProt / LSASS ---
  # LSASS as Protected Process Light (RunAsPPL)
  $ppl = Get-ItemPropertyValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' -Name 'RunAsPPL' -ErrorAction SilentlyContinue
  Add-Result -Id 'LSASS-01' -Priority 'High' -Area 'CredProt' -Check 'LSASS RunAsPPL' -Expected 'RunAsPPL=1' -Actual $ppl -Compliant (Test-ValueEq $ppl 1)

  # Disable WDigest (no storing reversible plaintext creds)
  $wd = Get-ItemPropertyValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\Wdigest' -Name 'UseLogonCredential' -ErrorAction SilentlyContinue
  Add-Result -Id 'LSASS-02' -Priority 'High' -Area 'CredProt' -Check 'Disable WDigest plaintext creds' -Expected 'UseLogonCredential=0 or not set' -Actual $wd -Compliant (($null -eq $wd) -or (Test-ValueEq $wd 0))

  # --- Network stack / Legacy protocols ---
  # SMBv1 disabled
  ...
  Add-Result -Id 'NET-01' -Priority 'High' -Area 'Network' -Check 'SMBv1 disabled (Server)' -Expected 'SMB1=0' -Actual $smb1 -Compliant (Test-ValueEq $smb1 0)

  # NTLM restrictions
  $lmComp = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' -Name 'LmCompatibilityLevel' -ErrorAction SilentlyContinue).LmCompatibilityLevel
  Add-Result -Id 'NET-02' -Priority 'High' -Area 'Network' -Check 'NTLM policy (LMCompatibilityLevel)' -Expected '>= 5' -Actual $lmComp -Compliant (($lmComp -as [int]) -ge 5)
  ...
  Add-Result -Id 'NET-03' -Priority 'Medium' -Area 'Network' -Check 'LLMNR disabled' -Expected 'EnableMulticast=0' -Actual $llmnr -Compliant (Test-ValueEq $llmnr 0)

  # NetBIOS over TCP/IP disabled on all adapters (audit)
  $nb = Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration | Where-Object {$_.IPEnabled} | Select-Object Description, TcpipNetbiosOptions
  $nbNonDisabled = @($nb | Where-Object { $_.TcpipNetbiosOptions -ne 2 })
  Add-Result -Id 'NET-04' -Priority 'Medium' -Area 'Network' -Check 'NetBIOS over TCP/IP disabled' -Expected 'TcpipNetbiosOptions=2 across adapters' -Actual ("Non-compliant adapters: {0}" -f $nbNonDisabled.Count) -Compliant ($nbNonDisabled.Count -eq 0)
  ...
  Add-Result -Id 'RDP-01' -Priority 'High' -Area 'Remote Access' -Check 'RDP requires NLA' -Expected 'UserAuthentication=1' -Actual $nla -Compliant (Test-ValueEq $nla 1)

  # RDP encryption level (High/128-bit)
  $enc = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp' -Name 'MinEncryptionLevel' -ErrorAction SilentlyContinue).MinEncryptionLevel
  Add-Result -Id 'RDP-02' -Priority 'High' -Area 'Remote Access' -Check 'RDP encryption >= High' -Expected 'MinEncryptionLevel >= 3' -Actual $enc -Compliant (($enc -as [int]) -ge 3)

  # --- Defender / ASR / SmartScreen ---
  # PUA protection
  $pua = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender' -Name 'PUAProtection' -ErrorAction SilentlyContinue).PUAProtection
  Add-Result -Id 'DEF-01' -Priority 'High' -Area 'Defender' -Check 'PUA protection enabled' -Expected 'PUAProtection=1' -Actual $pua -Compliant (Test-ValueEq $pua 1)

  # Cloud-delivered protection
  $cloud = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Spynet' -Name 'SubmitSamplesConsent' -ErrorAction SilentlyContinue).SubmitSamplesConsent
  Add-Result -Id 'DEF-02' -Priority 'Medium' -Area 'Defender' -Check 'Cloud sample submission (consent)' -Expected '>= 1' -Actual $cloud -Compliant (($cloud -as [int]) -ge 1)

  # Real-time monitoring
  $rt = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection' -Name 'DisableRealtimeMonitoring' -ErrorAction SilentlyContinue).DisableRealtimeMonitoring
  Add-Result -Id 'DEF-03' -Priority 'High' -Area 'Defender' -Check 'Real-time protection enabled' -Expected 'DisableRealtimeMonitoring=0' -Actual $rt -Compliant (Test-ValueEq $rt 0)

  # SmartScreen for Explorer
  $ss = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System' -Name 'EnableSmartScreen' -ErrorAction SilentlyContinue).EnableSmartScreen
  Add-Result -Id 'OS-SS-01' -Priority 'Medium' -Area 'OS' -Check 'SmartScreen enabled (Explorer)' -Expected 'EnableSmartScreen=1' -Actual $ss -Compliant (Test-ValueEq $ss 1)

  # --- Logging / Auditing ---
  # PowerShell script block logging
  $psbl = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging' -Name 'EnableScriptBlockLogging' -ErrorAction SilentlyContinue).EnableScriptBlockLogging
  Add-Result -Id 'LOG-PS-01' -Priority 'Medium' -Area 'Logging' -Check 'PowerShell Script Block Logging' -Expected 'EnableScriptBlockLogging=1' -Actual $psbl -Compliant (Test-ValueEq $psbl 1)

  # PowerShell module logging
  $psml = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ModuleLogging' -Name 'EnableModuleLogging' -ErrorAction SilentlyContinue).EnableModuleLogging
  Add-Result -Id 'LOG-PS-02' -Priority 'Medium' -Area 'Logging' -Check 'PowerShell Module Logging' -Expected 'EnableModuleLogging=1' -Actual $psml -Compliant (Test-ValueEq $psml 1)

  # Audit: Command line process creation
  $cl = (Get-ItemProperty -Path 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System\Audit' -Name 'ProcessCreationIncludeCmdLine_Enabled' -ErrorAction SilentlyContinue).ProcessCreationIncludeCmdLine_Enabled
  Add-Result -Id 'LOG-PRC-01' -Priority 'Medium' -Area 'Logging' -Check 'Include command line in process creation events' -Expected '..._Enabled=1' -Actual $cl -Compliant (Test-ValueEq $cl 1)

  # --- Disk / BitLocker ---
  # Require BitLocker for OS volume (presence audit; not enforcing encryption here)
  $bl = (Get-BitLockerVolume -MountPoint 'C:' -ErrorAction SilentlyContinue)
  $blEnabled = $false; if ($bl -and $bl.ProtectionStatus -eq 'On') { $blEnabled = $true }
  Add-Result -Id 'BL-01' -Priority 'High' -Area 'Disk' -Check 'BitLocker enabled for OS volume' -Expected 'ProtectionStatus=On' -Actual ($bl.ProtectionStatus) -Compliant $blEnabled

  # --- LDAP signing / channel binding (for domain-joined machines) ---
  try {
    $ldapSign = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\NTDS\Parameters' -Name 'LDAPServerIntegrity' -ErrorAction Stop).LDAPServerIntegrity
    Add-Result -Id 'LDAP-01' -Priority 'High' -Area 'Identity' -Check 'LDAP signing required (DC only)' -Expected 'LDAPServerIntegrity=2' -Actual $ldapSign -Compliant (Test-ValueEq $ldapSign 2)
  } catch {}

  # --- Print Spooler (PrintNightmare) ---
  $spoolSvc = (Get-Service -Name Spooler -ErrorAction SilentlyContinue)
  $spoolStart = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\Spooler' -Name 'Start' -ErrorAction SilentlyContinue).Start
  $allowRpc = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Printers' -Name 'RegisterSpoolerRemoteRpcEndPoint' -ErrorAction SilentlyContinue).RegisterSpoolerRemoteRpcEndPoint
  Add-Result -Id 'PRINT-01' -Priority 'Medium' -Area 'OS' -Check 'Spooler locked down (RPC endpoint not registered)' -Expected 'RegisterSpoolerRemoteRpcEndPoint=2' -Actual $allowRpc -Compliant (Test-ValueEq $allowRpc 2)

  # --- SMB signing (client) ---
  $requireSign = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanWorkstation\Parameters' -Name 'RequireSecuritySignature' -ErrorAction SilentlyContinue).RequireSecuritySignature
  Add-Result -Id 'SMB-CLI-01' -Priority 'High' -Area 'Network' -Check 'SMB client requires signing' -Expected 'RequireSecuritySignature=1' -Actual $requireSign -Compliant (Test-ValueEq $requireSign 1)

  # --- WDAC presence (policy audit) ---
  $wdac = Get-ChildItem -Path 'C:\Windows\System32\CodeIntegrity\CiPolicies\Active' -Filter '*.cip' -ErrorAction SilentlyContinue
  Add-Result -Id 'WDAC-01' -Priority 'Low' -Area 'OS' -Check 'WDAC active policy present' -Expected '*.cip present' -Actual ("Policies={0}" -f ($wdac | Select-Object -ExpandProperty Name -ErrorAction SilentlyContinue -First 1)) -Compliant ($wdac -ne $null -and $wdac.Count -ge 1)
}

# Hook into existing flow: run our additional checks before normalize/export, if not already called.
try {
  Invoke-AdditionalBaselineChecks
} catch {
  Write-Warning "Invoke-AdditionalBaselineChecks failed: $($_.Exception.Message)"
}
# ======= End Additional Baseline Checks =======


# ======= v3.4h Export-Results override (forces full schema) =======
# Remove existing Export-Results to avoid partial projections
if (Get-Command Export-Results -ErrorAction SilentlyContinue) {
  Remove-Item Function:\Export-Results -ErrorAction SilentlyContinue
}
...
function Export-Results {
  ...
  # Fallback to global vars if paths omitted
  if (-not $CsvPath -and (Get-Variable -Name CsvPath -Scope Global -ErrorAction SilentlyContinue)) { $CsvPath = $global:CsvPath }
  if (-not $JsonPath -and (Get-Variable -Name JsonPath -Scope Global -ErrorAction SilentlyContinue)) { $JsonPath = $global:JsonPath }

  # Ensure Prereq + CompliantBool + stable shape
  $Projected = @(
    foreach ($r in $Results) {
      $pr = $null
      if ($r.PSObject.Properties.Name -contains 'Prereq') { $pr = $r.Prereq } else { $pr = 'None' }
  ...
  if ($CsvPath) { $Projected | Export-Csv -Path $CsvPath -NoTypeInformation -Encoding UTF8 }
  if ($JsonPath) { $Projected | ConvertTo-Json -Depth 6 | Set-Content -Path $JsonPath -Encoding UTF8 }
  return $Projected
}
# ======= end v3.4h Export-Results override =======
